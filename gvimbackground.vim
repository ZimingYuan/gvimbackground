...
" Declaration of variable 'colors' below was modified from file '$VIM\rgb.txt'
let colors = {
    \ 'snow' : '#fffafa',
    \ 'ghost white' : '#f8f8ff',
    \ 'ghostwhite' : '#f8f8ff',
...
let color = synIDattr(hlID('Normal'), 'bg', 'gui')
" Change the color name to color value
if color[0] != '#'
    let color = tolower(color)
    if has_key(colors, color)
...
" If the red value, green value and blue value are equal will cause display
" error so I change the red value a bit.
if r == g && g == b
    if r == 0
        let r = r + 6
    else
